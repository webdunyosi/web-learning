...
const AdminSidebar = ({ isSidebarOpen, setIsSidebarOpen }) => {
  ...
  const navItems = [
    { name: 'Asosiy panel', path: '/admin/dashboard', icon: LayoutDashboard },
      ...
      >
        ...
        <div className="h-20 flex items-center justify-between px-6 border-b border-white/10 shrink-0 relative z-10">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-br from-rose-500 to-orange-600 rounded-xl flex items-center justify-center shadow-[0_0_20px_rgba(244,63,94,0.4)] border border-white/20">
              <ShieldAlert className="w-6 h-6 text-white drop-shadow-md" />
            </div>
            <span className="text-xl font-bold text-white tracking-tight drop-shadow-sm italic">Admin Hub</span>
          </div>
          <button 
            onClick={() => setIsSidebarOpen(false)} 
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-3.5 rounded-xl transition-all duration-300 relative overflow-hidden group ${
                  isActive 
                    ? 'text-rose-300 bg-white/[0.06] border border-white/10 shadow-[inset_0_0_20px_rgba(244,63,94,0.15)] backdrop-blur-md' 
                    : 'text-gray-400 hover:text-gray-200 hover:bg-white/[0.04] border border-transparent'
                }`}
              >
                {isActive && (
                  <div className="absolute left-0 top-0 bottom-0 w-1 bg-rose-400 shadow-[0_0_15px_rgba(244,63,94,1)] rounded-r-md"></div>
                )}
                <div className={`absolute inset-0 bg-gradient-to-r from-rose-500/0 to-rose-500/5 opacity-0 ${!isActive && 'group-hover:opacity-100'} transition-opacity duration-300 pointer-events-none`}></div>

                <Icon className={`w-5 h-5 transition-transform duration-300 ${isActive ? 'scale-110 drop-shadow-[0_0_8px_rgba(251,113,133,0.6)]' : 'group-hover:scale-110'}`} />
