...
const TeacherHeader = ({ setIsSidebarOpen }) => {
  ...
  const handleLogout = () => {
    // 1. Ma'lumotlarni tozalash (agar kerak bo'lsa)
  ...
  return (
    <header className="h-20 bg-white/[0.02] backdrop-blur-3xl border-b border-white/10 flex items-center justify-between px-4 sm:px-6 lg:px-8 sticky top-0 z-50">
      ...
      {/* 2. PROFIL VA BILDIRISHNOMALAR */}
      <div className="flex items-center gap-4 relative z-10">
        
        {/* Status Badge */}
        <div className="hidden xs:flex items-center gap-2.5 bg-indigo-500/5 border border-indigo-500/20 px-4 py-2 rounded-2xl">
          <GraduationCap className="w-5 h-5 text-indigo-400" />
          <div className="flex flex-col leading-tight">
            <span className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">Lavozim</span>
            <span className="text-sm font-bold text-indigo-300">O'qituvchi</span>
          </div>
        </div>
        ...
        <div className="relative" ref={profileRef}>
          ...
          >
            <div className="hidden lg:block text-right">
              <p className="text-sm font-bold text-gray-200 group-hover:text-white">Sardor Tursunov</p>
              <p className="text-[11px] text-indigo-400 font-medium">O'qituvchi</p>
            </div>
            
            <div className="relative">
              <div className="w-10 h-10 rounded-xl bg-white/[0.05] border border-white/20 p-[2px] shadow-[0_0_15px_rgba(255,255,255,0.05)] group-hover:border-indigo-400/50 transition-all">
                <div className="w-full h-full bg-gradient-to-br from-indigo-500/20 to-purple-500/10 rounded-[8px] flex items-center justify-center font-bold text-sm text-indigo-300 uppercase">
                  ST
                </div>
              </div>
